...
func _show_action_time_indicator(
	...
	var waiter_world_position = 		waiter_node.global_position

	var object_world_position = 		_get_action_target_world_position()

	var indicator_world_position = 		waiter_world_position.lerp(
			object_world_position,
			0.5
		)

	indicator_world_position.y -= 		action_time_indicator_vertical_offset
	...
	indicator.position = (
		to_local(indicator_world_position)
		- indicator.size * 0.5
	)

	var starting_position = indicator.position
	var ending_position = (
		starting_position
		+ Vector2(
			0,
...
func _get_action_target_world_position() -> Vector2:
	...
	var target_sprite = 		_find_first_sprite_recursive(
			action_time_target_node
		)
...
func _find_first_sprite_recursive(
	root_node: Node
) -> Sprite2D:
	for child in root_node.get_children():
		...
		var nested_sprite = 			_find_first_sprite_recursive(
				child
			)
