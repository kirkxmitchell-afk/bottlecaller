...
@export_category("Mise HUD")

## Assign the mise-en-place HUD icon supplied for:
## serviette + knife + fork.
@export var mise_icon_texture: Texture2D

@export_category("Split HUD Backgrounds")

## Optional background artwork for the Tier 2 unlock requirements HUD.
@export var tier_goal_hud_background_texture: Texture2D

## Optional background artwork for the immediate task / status HUD.
@export var live_status_hud_background_texture: Texture2D

@export_category("Split HUD Layout")

## Tall Tier-goals column placed on the left side of the restaurant.
## The default size matches the portrait objective-menu artwork.
@export var tier_goal_hud_position = Vector2(28, 112)
@export var tier_goal_hud_size = Vector2(300, 400)

## Square live-status menu placed near the right-side waiter/stations.
## The default size matches the square status-menu artwork.
@export var live_status_hud_position = Vector2(1080, 165)
@export var live_status_hud_size = Vector2(360, 360)
...
func _create_hud():
	...
	_create_unlock_requirements_hud()
	_create_live_status_hud()
	_create_mise_hud()
	_create_encounter_panel()
	_create_result_panel()


func _create_unlock_requirements_hud():
	unlock_panel = Panel.new()
	unlock_panel.name = "Tier2UnlockPanel"
	unlock_panel.position = tier_goal_hud_position
	unlock_panel.size = tier_goal_hud_size
	unlock_panel.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	unlock_panel.z_index = 900
	hud_root.add_child(
		unlock_panel
	)
	...
	_apply_panel_background(
		unlock_panel,
		tier_goal_hud_background_texture,
		Color(
			0.018,
			0.08,
			0.045,
	...
	var panel_width = unlock_panel.size.x
	var panel_height = unlock_panel.size.y

	unlock_title_label = Label.new()
	unlock_title_label.name = "Tier2UnlockTitle"
	unlock_title_label.text = "TIER 1 GOALS"
	unlock_title_label.position = Vector2(
		panel_width * 0.10,
		panel_height * 0.035
	)
	unlock_title_label.size = Vector2(
		panel_width * 0.80,
		panel_height * 0.065
	)
	unlock_title_label.horizontal_alignment = \
		HORIZONTAL_ALIGNMENT_CENTER
	unlock_title_label.vertical_alignment = \
		VERTICAL_ALIGNMENT_CENTER
	unlock_title_label.add_theme_font_size_override(
		"font_size",
		22
	)
	unlock_title_label.add_theme_color_override(
		"font_color",
		Color(1.0, 0.87, 0.48, 1.0)
	)
	unlock_title_label.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	unlock_panel.add_child(
		unlock_title_label
	)

	unlock_goal_label = Label.new()
	unlock_goal_label.name = "Tier2GoalLabel"
	unlock_goal_label.position = Vector2(
		panel_width * 0.10,
		panel_height * 0.125
	)
	unlock_goal_label.size = Vector2(
		panel_width * 0.80,
		panel_height * 0.10
	)
	unlock_goal_label.autowrap_mode = \
		TextServer.AUTOWRAP_WORD_SMART
	unlock_goal_label.horizontal_alignment = \
		HORIZONTAL_ALIGNMENT_CENTER
	unlock_goal_label.vertical_alignment = \
		VERTICAL_ALIGNMENT_CENTER
	unlock_goal_label.add_theme_font_size_override(
		"font_size",
		16
	)
	unlock_goal_label.add_theme_color_override(
		"font_color",
		Color(1.0, 0.82, 0.30, 1.0)
	)
	unlock_goal_label.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	unlock_panel.add_child(
		unlock_goal_label
	)

	unlock_tables_label = _create_tier_requirement_label(
		"Tier2TablesLabel",
		panel_height * 0.255
	)

	unlock_coins_label = _create_tier_requirement_label(
		"Tier2CoinsLabel",
		panel_height * 0.365
	)

	unlock_wines_label = _create_tier_requirement_label(
		"Tier2WinesLabel",
		panel_height * 0.475
	)

	unlock_station_label = _create_tier_requirement_label(
		"Tier2StationScoreLabel",
		panel_height * 0.585
	)

	unlock_progress_label = Label.new()
	unlock_progress_label.name = "Tier2ProgressLabel"
	unlock_progress_label.position = Vector2(
		panel_width * 0.10,
		panel_height * 0.755
	)
	unlock_progress_label.size = Vector2(
		panel_width * 0.80,
		panel_height * 0.12
	)
	unlock_progress_label.horizontal_alignment = \
		HORIZONTAL_ALIGNMENT_CENTER
	unlock_progress_label.vertical_alignment = \
		VERTICAL_ALIGNMENT_CENTER
	unlock_progress_label.autowrap_mode = \
		TextServer.AUTOWRAP_WORD_SMART
	unlock_progress_label.add_theme_font_size_override(
		"font_size",
		17
	)
	unlock_progress_label.add_theme_color_override(
		"font_color",
		Color(1.0, 0.87, 0.48, 1.0)
	)
	unlock_progress_label.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	unlock_panel.add_child(
		unlock_progress_label
	)


func _create_tier_requirement_label(
	label_name: String,
	y_position: float
) -> Label:
	var label = Label.new()
	...
	label.position = Vector2(
		unlock_panel.size.x * 0.12,
		y_position
	)
	label.size = Vector2(
		unlock_panel.size.x * 0.76,
		unlock_panel.size.y * 0.08
	)
...
func _create_live_status_hud():
	status_panel = Panel.new()
	status_panel.name = "LiveStatusPanel"
	status_panel.position = live_status_hud_position
	status_panel.size = live_status_hud_size
	status_panel.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	status_panel.z_index = 900
	hud_root.add_child(
		status_panel
	)
	...
	_apply_panel_background(
		status_panel,
		live_status_hud_background_texture,
		Color(
			0.015,
			0.08,
			0.045,
	...
	var panel_height = status_panel.size.y

	# Kept for the existing prompt system, but intentionally hidden.
	# The visible HUD contains only Mise, Task and Location.
	prompt_label = Label.new()
	prompt_label.name = "HiddenCurrentMessage"
	...
	status_title_label.name = "LiveStatusTitle"
	...
	# First green horizontal row.
	objective_label = Label.new()
	objective_label.name = "ImmediateObjective"
	objective_label.position = Vector2(
		panel_width * 0.13,
		panel_height * 0.515
	)
	objective_label.size = Vector2(
		panel_width * 0.74,
		panel_height * 0.145
	)
	objective_label.horizontal_alignment = \
		HORIZONTAL_ALIGNMENT_CENTER
	objective_label.vertical_alignment = \
		VERTICAL_ALIGNMENT_CENTER
	...
	objective_label.add_theme_constant_override(
		"outline_size",
		3
	)
	objective_label.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	status_panel.add_child(
		objective_label
	)

	# Second green horizontal row.
	service_label = Label.new()
	service_label.name = "ImmediateLocation"
	service_label.position = Vector2(
		panel_width * 0.13,
		panel_height * 0.755
	)
	service_label.size = Vector2(
		panel_width * 0.74,
		panel_height * 0.145
	)
	service_label.horizontal_alignment = \
		HORIZONTAL_ALIGNMENT_CENTER
	service_label.vertical_alignment = \
		VERTICAL_ALIGNMENT_CENTER
	service_label.autowrap_mode = \
		TextServer.AUTOWRAP_WORD_SMART
	service_label.add_theme_font_size_override(
		"font_size",
		16
	)
	service_label.add_theme_color_override(
		"font_color",
		Color(1.0, 0.84, 0.32, 1.0)
	)
	service_label.add_theme_color_override(
	...
	service_label.add_theme_constant_override(
		"outline_size",
		3
	)
	service_label.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	status_panel.add_child(
		service_label
	)

	# These variables remain available to the wider script but are not
	# shown in this compact status menu.
	carrying_label = Label.new()
	carrying_label.name = "HiddenCarryingLabel"
	...
	tables_label.name = "HiddenFocusLabel"
	tables_label.visible = false
	status_panel.add_child(
		tables_label
	)


func _apply_panel_background(
	panel: Panel,
	...
	if background_texture != null:
		# The supplied PNG already contains its own dark surface,
		# gold frame and transparency. A transparent Panel style avoids
		# the doubled black overlay seen in the previous version.
		...
		transparent_style.border_color = Color(
			0,
			0,
			0,
			0
		...
		background_rect.name = "PanelBackground"
		background_rect.position = Vector2.ZERO
		background_rect.size = panel.size
		...
		background_rect.mouse_filter = \
			Control.MOUSE_FILTER_IGNORE
		background_rect.modulate = Color(
			1,
			1,
			1,
			1
	...
	fallback_style.bg_color = fallback_bg
	fallback_style.border_color = border_color
	fallback_style.set_border_width_all(3)
	fallback_style.set_corner_radius_all(16)
	panel.add_theme_stylebox_override(
		"panel",
		fallback_style
	)
...
func _create_mise_hud():
	var panel_width = status_panel.size.x
	var panel_height = status_panel.size.y

	# The Mise alert occupies only the black upper square of the artwork.
	mise_icon = TextureRect.new()
	mise_icon.name = "MiseInventoryIcon"
	mise_icon.position = Vector2(
		panel_width * 0.115,
		panel_height * 0.065
	)
	mise_icon.size = Vector2(
		panel_width * 0.53,
		panel_height * 0.30
	)
	mise_icon.expand_mode = \
		TextureRect.EXPAND_IGNORE_SIZE
	mise_icon.stretch_mode = \
		TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	mise_icon.texture = mise_icon_texture
	mise_icon.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	...
	mise_fallback_label = Label.new()
	mise_fallback_label.name = "MiseFallbackIcon"
	mise_fallback_label.text = "M&E"
	mise_fallback_label.position = Vector2(
		panel_width * 0.115,
		panel_height * 0.095
	)
	mise_fallback_label.size = Vector2(
		panel_width * 0.53,
		panel_height * 0.20
	)
	mise_fallback_label.horizontal_alignment = \
		HORIZONTAL_ALIGNMENT_CENTER
	mise_fallback_label.vertical_alignment = \
		VERTICAL_ALIGNMENT_CENTER
	mise_fallback_label.add_theme_font_size_override(
		"font_size",
		28
	)
	mise_fallback_label.add_theme_color_override(
		"font_color",
		Color(1.0, 0.84, 0.32, 1.0)
	)
	mise_fallback_label.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
	...
	mise_label = Label.new()
	mise_label.name = "MiseInventoryLabel"
	mise_label.position = Vector2(
		panel_width * 0.075,
		panel_height * 0.355
	)
	mise_label.size = Vector2(
		panel_width * 0.61,
		panel_height * 0.10
	)
	mise_label.horizontal_alignment = \
		HORIZONTAL_ALIGNMENT_CENTER
	mise_label.vertical_alignment = \
		VERTICAL_ALIGNMENT_CENTER
	mise_label.autowrap_mode = \
		TextServer.AUTOWRAP_WORD_SMART
	mise_label.add_theme_font_size_override(
		"font_size",
		15
	)
	mise_label.add_theme_color_override(
		"font_color",
		Color(1.0, 0.84, 0.32, 1.0)
	)
	mise_label.add_theme_color_override(
	...
	mise_label.add_theme_constant_override(
		"outline_size",
		3
	)
	mise_label.mouse_filter = \
		Control.MOUSE_FILTER_IGNORE
