...
func _handle_table_arrival(
	...
	# Mise is a separate one-slot inventory.
	if (
		mise_inventory_filled
		and mise_inventory_table_id == table_id
	):
		_lay_mise_at_table(
			table_id
		)
...
func _set_greeting_buttons_visible(
	...
	greet_wine_button.visible = is_visible
	greet_aperitif_button.visible = is_visible
	greet_food_button.visible = is_visible


func _set_follow_up_buttons_visible(
	is_visible: bool
):
	walk_away_button.visible = is_visible
	offer_food_button.visible = is_visible
	offer_wine_button.visible = is_visible
...
func _handle_mise_arrival():
	if mise_inventory_filled:
		_set_prompt(
			"Mise slot is already filled for "
			+ mise_inventory_table_id
			+ "."
		)
		return

	var table_id = _resolve_mise_target_table()

	if table_id == "":
		_set_prompt(
			"Select an active food table or an empty table needing reset first."
		)
		return
	...
	mise_inventory_table_id = table_id
	mise_inventory_purpose = purpose

	_set_prompt(
		"Serviette, knife and fork collected for "
		+ table_id
		+ ". Lay them at that table."
	)

	_update_mise_icon()


func _resolve_mise_target_table() -> String:
	if table_sessions.has(
		focused_table_id
	):
		var session = table_sessions[
...
func _lay_mise_at_table(
	...
	if mise_inventory_table_id != table_id:
		_set_prompt(
			"This Mise belongs to "
			+ mise_inventory_table_id
			+ "."
		)
		return
	...
	var session = table_sessions[table_id]
	var table = session["table"]
	var purpose = mise_inventory_purpose
