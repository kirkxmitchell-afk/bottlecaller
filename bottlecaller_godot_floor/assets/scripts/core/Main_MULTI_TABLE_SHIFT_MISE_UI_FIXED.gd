...
func _handle_table_arrival(
	...
	# Mise is a separate one-slot inventory. It is intentionally
	# unassigned when collected, so the player chooses the receiving
	# table by clicking any table that currently needs it.
	...
	match phase:
		...
		"reset_required":
			_set_prompt(
				"Empty table needs resetting. Go to Mise en Place for a serviette, knife and fork."
			)
...
func _explain_active_table_tasks(
	...
	if session["chef_status"] == "ready":
		if bool(session["mise_set"]):
			...
			tasks.append(
				"lay Mise before collecting food"
			)
...
func _enter_order_at_pos(
	...
	_set_prompt(
		"Order entered for "
		+ table_id
		+ ". Bar and Chef tickets have been sent. Complete Mise before collecting food."
	)
...
func _collect_receipt_from_pos():
	...
	if mise_inventory_filled:
		_set_prompt(
			"Mise inventory is full. Give it to an eligible table before collecting the receipt."
		)
		return
...
func _handle_bar_arrival():
	...
	if mise_inventory_filled:
		_set_prompt(
			"Mise inventory is full. Give it to an eligible table before collecting the drink."
		)
		return
...
func _handle_chef_arrival():
	...
	if mise_inventory_filled:
		_set_prompt(
			"Mise inventory is full. Give it to an eligible table before collecting food."
		)
		return
	...
	if (
		...
		_set_prompt(
			"Food is ready, but "
			+ table_id
			+ " still needs Mise. Collect and lay the serviette, knife and fork first."
		)
		return
...
func _deliver_food(
	...
	if (
		...
		_set_prompt(
			"Lay Mise at this table before delivering food."
		)
		return
...
func _handle_mise_arrival():
	if mise_inventory_filled:
		_set_prompt(
			"Mise inventory is already full. Choose any eligible table and click it."
		)
		return
	...
	if required_count <= 0:
		_set_prompt(
			"No active table currently needs a serviette, knife and fork."
		)
		return
	...
	# The slot is deliberately generic. No table is assigned here.
	mise_inventory_filled = true
	mise_inventory_table_id = ""
	mise_inventory_purpose = ""

	_set_prompt(
		"Serviette, knife and fork collected. Choose which eligible table receives them."
	)
...
func _get_mise_purpose_for_table(
	table_id: String
) -> String:
	if not table_sessions.has(table_id):
		return ""
...
func _lay_mise_at_table(
	...
	if not _table_can_receive_mise(table_id):
		_set_prompt(
			table_id
			+ " does not currently need Mise. The inventory remains full; choose another eligible table."
		)
		return

	var session = table_sessions[table_id]
	var table = session["table"]
	var purpose = _get_mise_purpose_for_table(
		table_id
	)
	...
	if purpose == "reset":
		session = _create_ready_table_session(
			table
		)
		table_sessions[table_id] = session
	...
	_set_prompt(
		"Mise laid at "
		+ table_id
		+ ". Food may now be collected when Chef is ready."
	)
...
func _collect_dirty_plates(
	...
	if mise_inventory_filled:
		_set_prompt(
			"Mise inventory is full. Give it to an eligible table before clearing plates."
		)
		return
...
func _handle_scullery_arrival():
	if waiter_carrying != CARRY_DIRTY:
		_set_prompt(
			"Scullery currently needs dirty plates."
		)
		return
	...
	_set_prompt(
		"Plates dropped. Go to POS for the bill for "
		+ table_id
		+ "."
	)
...
func _enable_table_reset(
	...
	_set_prompt(
		table_id
		+ " now needs a serviette, knife and fork from Mise en Place."
	)
	...
	match phase:
		...
		"service_active":
			if (
				...
				message = "Mise required"
			elif session["bar_status"] == "ready":
				message = "Drink ready"
...
func _create_hud():
	...
	# One readable screen replaces the scattered world-overlay text.
	status_panel = Panel.new()
	status_panel.name = "GameStatusPanel"
	...
	status_style.border_color = Color(
		0.95,
		0.66,
		0.22,
		1.0
	...
	status_title_label.name = "GameStatusTitle"
	status_title_label.text = "GAME STATUS & OBJECTIVE"
	...
	tables_label.name = "GuestServicesLabel"
...
func _create_mise_hud():
	...
	# Visible fallback if the exported icon was reset or not assigned.
...
func _get_carrying_description() -> String:
	if waiter_carrying == CARRY_NONE:
		if mise_inventory_filled:
			return "Serviette, knife and fork — unassigned"

		return "Nothing"

	if carrying_table_id == "":
		return waiter_carrying

	return (
		waiter_carrying.replace("_", " ").capitalize()
		+ " for "
		+ carrying_table_id
	)
...
func _get_current_objective() -> String:
	...
	if waiter_carrying == CARRY_DIRTY:
		return "Take the dirty plates to Scullery."
	...
	if mise_inventory_filled:
		return "Choose any table needing cutlery or resetting, then click that table."
	...
	if chef_order_ready and not chef_current_order.is_empty():
		...
		if table_sessions.has(chef_table_id):
			var chef_session = table_sessions[
				chef_table_id
			]

			if (
				bool(chef_session["mise_required"])
				and not bool(chef_session["mise_set"])
			):
				return (
					"Collect Mise and lay it at "
					+ chef_table_id
	...
	for table_id in table_sessions.keys():
		...
		if phase == "reset_required":
			return "Collect Mise and reset an empty table."
