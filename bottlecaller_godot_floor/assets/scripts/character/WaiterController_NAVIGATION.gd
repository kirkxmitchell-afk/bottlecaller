...
var path_desired_distance = 16.0

@export_range(2.0, 100.0, 1.0)
var target_desired_distance = 14.0

@export_range(1, 180, 1)
var navigation_map_wait_frames = 30

@export var clamp_targets_to_navigation = true
@export var use_direct_fallback = true
...
func _configure_navigation_agent():
	if navigation_agent == null:
		push_warning(
			...
			+ "Direct fallback movement will be used."
		)
		return
...
func _update_waiting_for_navigation_map():
	...
	if use_direct_fallback:
		...
		_fail_navigation(
			"Navigation map is unavailable or has no baked polygon."
		)
