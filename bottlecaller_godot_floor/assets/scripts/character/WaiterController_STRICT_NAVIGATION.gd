...
const WAITER_SOURCE_WIDTH = 1920.0
const WAITER_SOURCE_HEIGHT = 1080.0
...
const WAITER_VISUAL_METRICS = {
	"idle_empty_down": Vector2(835.0, 957.0),
	"idle_empty_down_left": Vector2(843.0, 944.0),
	"idle_empty_down_right": Vector2(805.0, 972.0),
	"idle_empty_left": Vector2(806.0, 962.0),
	"idle_empty_right": Vector2(802.0, 953.0),
	"idle_empty_up": Vector2(860.0, 961.0),
	"idle_empty_up_left": Vector2(868.0, 988.0),
	"idle_empty_up_right": Vector2(899.0, 979.0),
	"walk_aperitif_down": Vector2(715.0, 880.0),
	"walk_aperitif_down_left": Vector2(720.0, 900.0),
	"walk_aperitif_down_right": Vector2(720.0, 900.0),
	"walk_aperitif_left": Vector2(678.0, 834.0),
	"walk_aperitif_right": Vector2(678.0, 834.0),
	...
	"walk_empty_down_right": Vector2(905.0, 998.0),
	"walk_empty_left": Vector2(951.0, 1021.0),
	"walk_empty_right": Vector2(951.0, 1021.0),
	"walk_empty_up": Vector2(929.0, 1006.0),
	"walk_empty_up_left": Vector2(812.0, 919.0),
	"walk_empty_up_right": Vector2(812.0, 919.0),
	"walk_food_plate_down": Vector2(877.0, 1029.0),
	"walk_food_plate_down_left": Vector2(725.0, 938.0),
	"walk_food_plate_down_right": Vector2(725.0, 938.0),
	"walk_food_plate_left": Vector2(814.0, 982.0),
	"walk_food_plate_right": Vector2(814.0, 982.0),
	"walk_food_plate_up": Vector2(892.0, 1006.0),
	"walk_food_plate_up_left": Vector2(782.0, 914.0),
	"walk_food_plate_up_right": Vector2(782.0, 914.0),
	"walk_receipt_down": Vector2(809.0, 945.0),
	"walk_receipt_down_left": Vector2(786.0, 928.0),
	"walk_receipt_down_right": Vector2(786.0, 928.0),
	"walk_receipt_left": Vector2(769.0, 939.0),
	"walk_receipt_right": Vector2(769.0, 939.0),
	"walk_receipt_up": Vector2(851.0, 938.0),
	"walk_receipt_up_left": Vector2(866.0, 951.0),
	"walk_receipt_up_right": Vector2(866.0, 951.0),
	"walk_wine_bottle_down": Vector2(778.0, 925.0),
	"walk_wine_bottle_down_left": Vector2(935.0, 978.0),
	"walk_wine_bottle_down_right": Vector2(935.0, 978.0),
	"walk_wine_bottle_left": Vector2(939.0, 1007.0),
	"walk_wine_bottle_right": Vector2(939.0, 1007.0),
	"walk_wine_bottle_up": Vector2(800.0, 928.0),
...
var walk_animation_fps = AUTHORED_WALK_ANIMATION_FPS
...
func _configure_walk_animation_fps():
	...
	for animation_name in sprite.sprite_frames.get_animation_names():
		if str(animation_name).begins_with("walk_"):
...
## Point the waiter at a world position, then refresh idle/walk pose.
func face_towards(world_position: Vector2, refresh_pose: bool = true):
...
func _apply_animation(
	...
	_apply_waiter_visual_normalization(str(animation_name))
...
func _apply_waiter_visual_normalization(animation_name):
	var metrics_name = animation_name.replace(
		"walk_reciept_",
		"walk_receipt_"
	)

	if not WAITER_VISUAL_METRICS.has(metrics_name):
		sprite.scale = _base_sprite_scale
		sprite.offset = _base_sprite_offset
		return

	var metrics: Vector2 = WAITER_VISUAL_METRICS[metrics_name]
	var source_height = metrics.x
	var source_bottom = metrics.y
	...
	var scale_multiplier = \
		WAITER_REFERENCE_HEIGHT / source_height
	var import_scale = \
		WAITER_IMPORT_WIDTH / WAITER_SOURCE_WIDTH
	var source_bottom_from_center = (
		source_bottom
		- (WAITER_SOURCE_HEIGHT * 0.5)
	) * import_scale
	var reference_foot_position = (
		(
			WAITER_REFERENCE_BOTTOM
			- (WAITER_SOURCE_HEIGHT * 0.5)
		)
		* import_scale
		+ _base_sprite_offset.y
	)

	sprite.scale = _base_sprite_scale * scale_multiplier
	sprite.offset = Vector2(
		_base_sprite_offset.x,
		(
...
func _build_idle_animation_options(direction):
	...
	# Dedicated idle clips first — never prefer a walk freeze when idle exists.
	_append_unique(
		options,
		"idle_"
		+ carrying_visual
