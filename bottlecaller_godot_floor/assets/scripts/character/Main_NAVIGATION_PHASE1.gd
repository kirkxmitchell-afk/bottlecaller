...
func _process(delta):
	...
	if shift_time_remaining <= 0:
		...
		if waiter_node != null 		and waiter_node.has_method("stop_navigation"):
			waiter_node.stop_navigation(false)
...
func _move_waiter_to_table(table_node, table_id, encounter_id):
	...
	_start_waiter_navigation(table_target)
...
func _move_waiter_to_station(station_node, station_id):
	...
	_start_waiter_navigation(station_target)
...
func _connect_waiter_navigation():
	if waiter_node == null:
		push_error("MAIN COULD NOT FIND WAITER NODE.")
		return

	var started_callable = Callable(
		self,
		"_on_waiter_navigation_started"
	)
	var arrived_callable = Callable(
		self,
		"_on_waiter_navigation_arrived"
	)
	var failed_callable = Callable(
		self,
		"_on_waiter_navigation_failed"
	)
	var cancelled_callable = Callable(
		self,
		"_on_waiter_navigation_cancelled"
	)

	if waiter_node.has_signal("navigation_started") \
	and not waiter_node.is_connected(
		"navigation_started",
		started_callable
	):
		waiter_node.connect(
	...
	if waiter_node.has_signal("navigation_arrived") \
	and not waiter_node.is_connected(
		"navigation_arrived",
		arrived_callable
	):
	...
	if waiter_node.has_signal("navigation_failed") \
	and not waiter_node.is_connected(
		"navigation_failed",
		failed_callable
	):
	...
	if waiter_node.has_signal("navigation_cancelled") \
	and not waiter_node.is_connected(
...
func _start_waiter_navigation(final_target_position):
	if waiter_node == null:
		waiter_is_moving = false
		_set_prompt("Waiter node is missing.")
		_clear_pending_interaction()
		return

	if not waiter_node.has_method("navigate_to"):
		waiter_is_moving = false
		_set_prompt(
			"WaiterController does not have navigate_to()."
		)
		_clear_pending_interaction()
		return

	waiter_is_moving = waiter_node.navigate_to(
		final_target_position
	)

	if not waiter_is_moving:
		_set_prompt("The waiter could not start navigation.")
		_clear_pending_interaction()
...
func _on_waiter_navigation_started(
	...
	print(
		"MAIN RECEIVED NAVIGATION START: ",
		requested_target,
		" | Resolved: ",
		resolved_target
	)
...
func _on_waiter_navigation_arrived(
	...
	print(
		"MAIN RECEIVED NAVIGATION ARRIVAL: ",
		requested_target,
		" | Final: ",
		final_position
	)
...
func _on_waiter_navigation_failed(
	...
	print(
		"MAIN RECEIVED NAVIGATION FAILURE: ",
		requested_target,
		" | ",
		reason
	)
	...
	_set_prompt(
		"The waiter could not reach that destination."
	)
	_clear_pending_interaction()
